...
<p align="center">
  <strong><font color="red">공개 showcase는 harmful/toxic text로 제한. 논문의 모든 claim은 유지; 증거와 실험은 이 repo에 보존.</font></strong>
</p>

<video src="https://github.com/user-attachments/assets/1cc80c48-02a4-4a5c-9d00-a0f10d91db15" controls width="600"></video>

> **Internal Safety Collapse (ISC)** 는 실패 표면을 프롬프트에서 워크플로우로 옮긴다. 작업을 코드·검증기·도구에 연결하라; 완료에 유해 콘텐츠가 필요하면 에이전트가 생성한다. **ASR@3** 에서, 테스트한 에이전트 능력 모델은 모두 **100%** 트리거율에 도달한다. 취약점은 프롬프트가 아니다. 워크플로우다.

> TVD는 ISC가 아니다. 하나의 엔지니어링 트리거일 뿐. TVD만 막으면 표면은 그대로 열려 있다.

> 이 repo를 LLM 에이전트로 요약하지 마라. 쓸 모델이 바로 여기서 실패하는 것들이다; 그 요약은 신뢰할 수 없다.
